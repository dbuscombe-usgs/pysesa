...
cdef class spectral:
   ...
   '''
   cdef object data, r, moments, lengthscale, lengths, psdparams #x_space, k_space,
   ...
   def __init__(self, np.ndarray[np.float64_t, ndim=2] points, int nbin=20, double res=0.05, int proctype=1, int lentype=1, int taper=1, str method='nearest'):
      ...
      if np.sum(np.isnan(im)) == np.prod(np.shape(im)):
         ...
         try:
            ...
            s, k  = self._psd(im, nx, ny, nbin, res, proctype) #k_back, s_b
...
#            return
   ...
   cpdef list _wav_rms(self, np.ndarray[np.float64_t, ndim=1] k, np.ndarray[np.float64_t, ndim=1] s, double res): #np.ndarray[np.float64_t, ndim=1] s_b,
      ...
      s_b = self._movingaverage(s, len(s)/5)
   ...
   @cython.wraparound(False)
   @cython.nonecheck(False)
   def _Hann2D(self, np.ndarray[np.float32_t, ndim=2] im):
      '''
      ...
      cdef np.ndarray[np.float32_t, ndim=2] res = np.empty((nx,ny),dtype=np.float32)

      a = nx/2+1
      b = ny/2+1
      X, Y = np.meshgrid(np.r_[:nx],np.r_[:ny])
      theta = np.asarray(X==a,'int') * (np.pi/2) + np.asarray(X!=a,'int') * np.arctan2( (Y-b),(X-a) ) # angular polar coordinate
      r = np.sqrt((Y-b)**2 + (X-a)**2) # radial polar coordinate
      rprime = np.sqrt((a**2)*(b**2)*(b**2*(np.cos(theta))**2 + a**2*(np.sin(theta))**2)**(-1)) # 'radius' of ellipse for this theta
      hanncoeff = (r < rprime)*(0.5*(1 + np.cos(np.pi*r/rprime)))

      res= np.asarray(im*hanncoeff,np.float32)
