...
#
      ...
      dist, indices = mytree.query(p, int(mxpts))#, distance_upper_bound=win)
...
#      cdef np.ndarray[np.float32_t, ndim=2] p = np.empty((lenx*leny,2), dtype=np.float32)
      ...
      #             print "memory error, using %s max points" % (str(mxpts))
         ...
         #          mytree = KDTree(toproc[:,:2].astype('float64'), leafsize=len(dat)/100) # adding this leafsize option speeds up considerably
